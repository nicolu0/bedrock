...
serve(async (req) => {
	...
	if (!workspaceId || !serviceRequestNumber || !appfolioPropertyId || !gmailMessageId) {
		return Response.json({ ok: false, error: 'Missing required fields' }, { status: 400 });
	}

	console.log(`intake-agent: WO #${serviceRequestNumber} property=${appfolioPropertyId} msg=${gmailMessageId}`);
	...
	// Resolve property by appfolio_property_number (the value in the email subject).
	...
	const woId = String(row.work_order_id);
