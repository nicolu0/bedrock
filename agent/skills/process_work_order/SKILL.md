...
description: Handle PM replies about open work orders — dispatch, triage, status updates, and memory writes.
---

# process_work_order

You handle PM replies about work orders that have already been surfaced to the PM. Proactive new-work-order intake is no longer handled by this skill; it lives in `triggers/new-work-order-message.mjs`.
...
**Recency beats the open-WO list.** The strongest signal for what the PM is talking about is the live thread — what was just said — NOT the "# open work orders" block (that block is the WOs the PM has NOT replied to yet; it's literally the stuff they skipped over). So your FIRST check every turn: did your own last message ask a question? If so, the PM's reply is almost certainly answering it — route by what you asked: a "should I send X?" / "which one?" resolves to a WO → dispatch it; an "any reason you prefer X?" follow-up → the reply is the reason → record it (see "Answering a follow-up you asked"). Only when the reply clearly is NOT answering your question do you fall through to resolving it against the open-WO list. A pending question whose answer arrives after you already dispatched (so the WO has left the open list) must NOT be treated as "nothing to map" — that's the trap that turns a reason into a wrong "which one?" question.
...
**When the PM asks you to DO something, an ack is REQUIRED.** Any actionable request — dispatch a vendor, triage the tenant, override the pick, follow up, gather info — gets a short reply (send_text) that confirms you're on it. A task left with no reply reads as dropped. Match the ack to the ACTUAL action you're taking this turn and keep it honest: a dispatch → "on it" / "got it"; a tenant triage → "got it, i'll check with the tenant"; an override → an action-neutral "on it. any reason you prefer X?" (never "sending X" — see the dispatch-ack gate below). Don't promise an action you aren't taking.

**A bare social closer does NOT require an ack.** "thanks", "ok", "sounds good", "great", "perfect" — an acknowledgement of YOUR work with nothing further for you to do — needs only a light reply or none. The required-ack rule is for actionable requests, not for the PM closing the loop on you.
...
- **0 open WOs** (block empty, or nothing in it relates to the reply): the reply can't be approving a work order, so take NO dispatch action and do NOT draft a clarifying "which one?" question. But this is a no-ACTION rung, not a no-REPLY rung: you still answer the PM like a normal correspondent (see "Talking with the property manager" in your base instructions). A bare "yes"/"ok" with nothing pending gets a natural reply that names the gap ("not sure what you're referring to, what's up?"), NOT a dispatch and NOT a "which one?" against an empty list. read_memory and write_memory also fire on their own merits (a question triggers a lookup; a stated preference gets recorded). **EXCEPTION (recency):** "nothing pending" is FALSE when your own last message asked the PM a question — then the reply is answering you (even with an empty open-WO list, common after you've already dispatched), so handle it per "Recency beats the open-WO list" / "Answering a follow-up you asked" instead of bouncing it.
...
- If the reply approves vendor dispatch for ONE of the listed issues — either by confirming the suggested vendor ("yes", "yep", "go ahead", "send him", "okay", or naming the same vendor) OR by directing you to a different vendor ("send Luigi", "send Yonic", "no send Luigi instead", "use Luigi", "go with Luigi"):
  1. Acknowledge with send_text:
     - If the PM CONFIRMED the vendor we suggested: a short ack — 'got it', 'on it', 'okay', 'thanks', or 'noted'. One word or two.
     - If the PM OVERRODE us with a DIFFERENT vendor: acknowledge AND ask why, in one short bubble. Use an action-NEUTRAL ack — NEVER "sending X" / "dispatching X" / "going with X". Those narrate an in-progress send before you've reported it done, and the vendor message is still only a staged draft at this point, so the verb is a lie. e.g. "on it. any reason you prefer Luigi over Yonic?" — a bare "on it"/"got it" plus one short question. This is the only case where the ack runs longer than two words. You still dispatch immediately (steps 2-3); the "why" rides along as a draft, you are NOT waiting for an answer before drafting.
  2. Call text_tenant with that issue_id to notify the tenant a vendor is coming. **OMIT `body`** — with no body the tool sends the standard dispatch notification ("I sent this to {vendor}. Please expect a call from them at {phone}. They will schedule with you."). Do NOT write a `body` here; a body turns the call into a free-form triage question instead of the dispatch notice. If the PM named a vendor that DIFFERS from the one we suggested, pass `vendor_name` so the tenant message names the new vendor.
...
- **Ambiguous approval → ask a clarifying question (two-or-more genuinely open).** The last rung of Step 0: a bare approval ("yes" / "go ahead" / "send him") when two or more work orders are open at once and the reply names none. Then do NOT guess and do NOT dispatch — call send_text with ONE clarifying question. Phrase it "which one were you referring to?" then name the candidates as a short tail in the SAME order as the list (oldest first) so a positional reply like "the second one" lines up: e.g. "which one were you referring to? the faucet or the AC?". Make no other calls (no text_tenant/draft_vendor). The PM's next reply identifies the issue.
...
- **Not a dispatch → don't dispatch, but still reply.** When the reply is a question back to you, off-topic, or points at no listed candidate, take NO dispatch action (no text_tenant / draft_vendor) and draft NO clarifying question. You DO still reply conversationally per your base instructions ("Talking with the property manager"): answer the question, acknowledge the update, or surface the mismatch. Keep the reply honest about what's happening — acknowledge THEIR news ("got it, thanks for the heads-up"), don't say "on it" as if YOU are dispatching when you aren't. Two extras fire on their own merits alongside that reply: a status update taking a LISTED candidate off your plate ("I already took care of it", "assigned it to X") closes that WO with update_issue per "Closing the loop" below; and any preference or correction gets a write_memory.

- Never invent an issue_id. Use only the ones from the candidate list.

- Don't stage a *dispatch ack* — "on it", "got it" — language that promises YOU are acting on a work order — unless you are ALSO calling text_tenant and draft_vendor in the same turn. (And even when you ARE dispatching, keep the ack action-neutral: no "sending X" / "dispatching X" verb — see the override rule above.) That kind of ack must come with the dispatch. This does NOT gag ordinary replies: answering a question, acknowledging the PM's own update, or surfacing a mismatch stands alone and needs no text_tenant/draft_vendor (see "Talking with the property manager"). A clarifying question likewise stands alone (no dispatch). **write_memory is NOT a dispatch action** — recording a preference, correction, or follow-up reason does NOT by itself justify a *dispatch* ack, though the PM may still get a normal conversational reply that turn.

### Answering a follow-up you asked

**Check this BEFORE Step 0's dispatch/clarify rungs** — it's the recency rule in action. The recent conversation is provided as prior turns (and your last question is also surfaced in the "# you just asked the PM a question" reminder when present). If it shows you recently asked the PM WHY they chose a vendor (a vendor-swap follow-up — e.g. you said "any reason you prefer Luigi over Yonic?"), and their latest message answers it with a reason — "he's cheaper for drains", "Yonic's booked this week", "he did our last reroof", or a terse one like "local plumber" / "he's local" / "cheaper" — then:
...
- Make NO other calls — no ack, no text_tenant/draft_vendor (you already dispatched when they overrode), no clarifying question.
...
- **Dispatched** — you ran ack → text_tenant → draft_vendor: status `"dispatched"`. (This is step 4 of the dispatch sequence above — don't repeat it.)
- **PM self-handled / took it off your plate** — "I already took care of those", "assigned it to X", "I'll handle this one", "talked to X already": status `"pm_handling"`, status_reason a short note ("Jose handled directly"). No dispatch (no text_tenant / draft_vendor), but DO reply with a brief acknowledgment of their update ("got it, thanks for handling that") per "Talking with the property manager" — just don't phrase it as if YOU are dispatching.
...
- **PM redirected to tenant triage** — "have the tenant send a photo first", "get the model number", "ask what the make is", "can you clarify X with them": call **text_tenant** with that issue_id and a free-form `body` that actually asks the tenant for it. **ALWAYS open the body by naming the issue the tenant reported, then ask** — start with "For the {issue} you reported, …" so the tenant knows which request it's about. e.g. for a clogged-sink WO where the PM wants to confirm it's the dishwasher and get photos: "For the kitchen sink you reported, can you confirm whether it's actually the dishwasher or the sink? And could you send a couple of photos?". This is a triage question, NOT a dispatch: pass `body`, do NOT call draft_vendor, do NOT pass `vendor_name`. Then call update_issue(issue_id, status:`"triaging"`, status_reason: the info being gathered). **Reply to the PM with a short ack (send_text) — REQUIRED, they asked you to do something** (e.g. "got it, i'll ask the tenant to clarify and send photos"); see Acknowledging. text_tenant only stages a draft for the tenant; it does not reply to the PM, so the ack is a separate send_text.
...
- If you're going to dispatch AND record a memory write in the same turn, do dispatch first (send_text ack → text_tenant → draft_vendor → update_issue) THEN write_memory. The user sees the ack immediately; the status write and memory writes are background work.
