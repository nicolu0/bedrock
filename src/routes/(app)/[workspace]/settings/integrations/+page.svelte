<script>
	// @ts-nocheck
	export let data;
	const { workspace, gmailConnection } = data;
	...
	const expiresLabel = gmailConnection?.expires_at ? formatDate(gmailConnection.expires_at) : null;
	const isConnected = Boolean(gmailConnection?.id);
	const connectionLabel = gmailConnection?.email ?? 'Not connected';
</script>
	...
	<section class="rounded-2xl border border-neutral-200 bg-white p-6">
		<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
			<div>
				...
				<div class="mt-4 text-sm text-neutral-600 md:mt-3">
					<div class="flex items-center gap-2">
						<span
							class={`h-2.5 w-2.5 rounded-full ${isConnected ? 'bg-emerald-500' : 'bg-neutral-300'}`}
						></span>
					...
					<div class="mt-1 text-xs text-neutral-400">
						{connectionLabel}
						{#if expiresLabel}
							<span class="ml-2">Expires {expiresLabel}</span>
						{/if}
					</div>
				</div>
			</div>
			<div class="flex flex-wrap items-center gap-2">
				<a
					href={`/${workspace.slug}/settings/integrations/gmail/connect`}
